---
name: osdu
description: "Analyzes OSDU platform health including CI/CD pipelines, test reliability, contributions, and GitLab operations. Provides live platform access via osdu extension when configured. Use when querying pipeline status, test pass rates, flaky tests, open MRs, contributor rankings, OSDU service metrics, or live platform data (records, schemas, entitlements)."
---
...
**CRITICAL RULE: Every sub-agent interaction MUST use the `task` tool to spawn a real agent. You MUST call the `task` tool — never simulate, role-play, or inline a sub-agent's work. No exceptions.**

---

## Scope

OSDU platform services on GitLab (`community.opengroup.org/osdu/platform`) only. This agent does NOT cover cimpl-azure IaC — that is @cimpl's domain.

- **"my MRs"** → `osdu-activity mr --involvement` (OSDU platform)
- **"pipeline status"** → `osdu-activity pipeline` (OSDU platform)
- For cimpl-azure queries, use base Copilot context with `glab` directly.
...
| Test execution ("run tests", "smoke test") | Spawn qa-runner sub-agent |
| Test failures ("why did tests fail", "analyze failures") | Spawn qa-analyzer sub-agent |
| Environment comparison ("compare", "diff environments") | Spawn qa-comparator sub-agent |
| Report generation ("generate report", "dashboard") | Spawn qa-reporter sub-agent |
| Build execution ("build", "compile", "verify") | Spawn build-runner sub-agent |
...
**Skill-aware routing:** Before spawning, check available skills for ones relevant to the task domain. If a matching skill exists, add to the spawn prompt: `Relevant skill: {skill-name} — read before starting.`
...
| Mode | When | How |
|------|------|-----|
| **Direct** | Status checks, factual questions, simple answers from context | Answer directly — NO agent spawn |
| **Lightweight** | Single-repo operations, simple scoped queries, small fixes | Spawn ONE agent with minimal prompt, or use `agent_type: "explore"` for read-only |
| **Standard** | Normal tasks, single-agent work requiring full context | Spawn one agent with full context — charter, skill refs, workspace root |
...
- "Run smoke tests on azure/ship" -> Spawn qa-runner with minimal prompt.
- "What's the build status?" -> `agent_type: "explore"` to check quickly.
...
Sub-agents are available as agents in this plugin — use the `task` tool to spawn them.
...
description: "{agent-name}: {brief task summary}"
prompt: |
  ...
  **Requested by:** {user name}
  ...
  Read your charter: agents/{agent-name}.agent.md
  ...
  RESPONSE ORDER:
  After ALL tool calls are complete, write a plain text summary as your
...
```
agent_type: "explore"
description: "{brief query}"
prompt: |
  Answer this question about the OSDU workspace at {osdu_workspace}:
  {question}
```
...
- If running tests -> also spawn qa-analyzer to be ready for failure diagnosis
...
|-----------|---------------------|
| Agent B needs Agent A's output | Hard data dependency |
| The user asked a direct question and is waiting | Direct interaction |
| A workflow phase gates the next phase | Sequential workflow |

**Use `mode: "background"` when:**
- Multiple agents can work in parallel
- The work is fire-and-forget (reporting, logging)
...
1. **CLI mode** — `task` tool is available -> full spawning control. Use `task` with `agent_type`, `mode`, `model`, `description`, `prompt` parameters.
2. **VS Code mode** — `runSubagent` or `agent` tool is available -> use `runSubagent` with the task prompt. Drop `agent_type`, `mode`, and `model` parameters.
3. **Fallback mode** — neither available -> work inline. Do not apologize. Execute the task directly.
...
5. **Execute** — Spawn agents, run skills, follow workflows
